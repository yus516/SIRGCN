...
ap.add_argument('--sim_mat', type=str, default='state-adj-49', help="adjacency matrix filename (*-adj.txt)")
ap.add_argument('--node_size', type=int, default=49, help="number of node")
...
ap.add_argument('--epochs', type=int, default=2000, help='number of epochs to train')
ap.add_argument('--lr', type=float, default=1e-2, help='initial learning rate')
ap.add_argument('--weight_decay', type=float, default=5e-5, help='weight decay (L2 loss on parameters).')
...
ap.add_argument('--horizon', type=int, default=1, help='leadtime default 5')
...
num_population = data_loader.population

if args.ablation is None:
    model = SIR_GNN(args.gpu, node_size=args.node_size, num_population=num_population, data_loader=data_loader)
else:
...
def evaluate(data_loader, data, tag='val', show=0):
    ...
    if (tag == 'full'):
        np.save('pred_full_sirgcn-japan.npy', y_pred_states)
        np.save('real_full_sirgcn-japan.npy', y_true_states)

    elif (tag == "test"):
        np.save('test_pred_winter-summer.npy', y_true_states)
        np.save('test_real_winter-summer.npy', y_pred_states)
    ...
    rmspe = np.sqrt(np.mean(np.square((y_pred[mask] - y_true[mask]) / np.square(y_true[mask]))))
    if not args.pcc:
    ...
    return float(total_loss / n_samples), mae,std_mae, rmse, rmse_states, pcc, pcc_states, mape, rmspe, r2, r2_states, var, var_states, peak_mae
...
try:
    ...
    for epoch in range(1, args.epochs+1):
        ...
        val_loss, mae,std_mae, rmse, rmse_states, pcc, pcc_states, mape, rmspe, r2, r2_states, var, var_states, peak_mae = evaluate(data_loader, data_loader.val)
        ...
        if val_loss < best_val:
            ...
            test_loss, mae,std_mae, rmse, rmse_states, pcc, pcc_states, mape, rmspe, r2, r2_states, var, var_states, peak_mae  = evaluate(data_loader, data_loader.test,tag='test')
            print('TEST MAE {:5.4f} std {:5.4f} RMSE {:5.4f} RMSEs {:5.4f} PCC {:5.4f} PCCs {:5.4f} MAPE {:5.4f} RMSPE {:5.4f} R2 {:5.4f} R2s {:5.4f} Var {:5.4f} Vars {:5.4f} Peak {:5.4f}'.format( mae, std_mae, rmse, rmse_states, pcc, pcc_states, mape, rmspe, r2, r2_states, var, var_states, peak_mae))
        else:
            bad_counter += 1
...
train_loss, mae,std_mae, rmse, rmse_states, pcc, pcc_states, mape, rmspe, r2, r2_states, var, var_states, peak_mae  = evaluate(data_loader, data_loader.train,tag='train',show=args.result)
print('Train MAE {:5.4f} std {:5.4f} RMSE {:5.4f} RMSEs {:5.4f} PCC {:5.4f} PCCs {:5.4f} MAPE {:5.4f} RMSPE {:5.4f} R2 {:5.4f} R2s {:5.4f} Var {:5.4f} Vars {:5.4f} Peak {:5.4f}'.format( mae, std_mae, rmse, rmse_states, pcc, pcc_states, mape, rmspe, r2, r2_states, var, var_states, peak_mae))


test_loss, mae,std_mae, rmse, rmse_states, pcc, pcc_states, mape, rmspe, r2, r2_states, var, var_states, peak_mae  = evaluate(data_loader, data_loader.test, tag='test',show=args.result)
test_loss, mae,std_mae, rmse, rmse_states, pcc, pcc_states, mape, rmspe, r2, r2_states, var, var_states, peak_mae  = evaluate(data_loader, data_loader.full, tag='full',show=args.result)
